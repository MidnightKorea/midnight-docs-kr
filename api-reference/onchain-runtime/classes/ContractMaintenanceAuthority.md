...
이 컨트랙트를 변경할 수 있는 권한을 가진 위원회입니다. 위원회의 공개 키 중 임계값 이상이
서명하면 컨트랙트의 규칙을 변경하거나 새 버전으로 다시 컴파일할 수 있습니다.
...
`counter`를 지정하지 않으면 기본값은 `0n`입니다. 값은
음수가 아니어야 하며 최대 2^32 - 1이어야 합니다.

배포 시 `counter`는 반드시 `0n`이어야 하며, 이후 업데이트 시에는
현재 값보다 정확히 1 큰 값으로 설정해야 합니다.
