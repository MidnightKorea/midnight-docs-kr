...
title: Web3 기초부터 배우기 — 해시 함수와 전자 서명
authors: samantha
...
1. **서명(Signing)**: 발신자가 원본 메시지를 해싱하고, 그 해시를 비밀 키로 암호화하여 **전자 서명**을 생성합니다.  
...
PGP로 암호화된 이메일을 보내면, 이메일 클라이언트가 비밀 키로 메시지에 서명합니다. 수신자는 발신자의 공개 키로 정말 발신자가 보낸 것인지, 전송 중 변경되지 않았는지 확인할 수 있습니다. 블록체인의 전자 서명도 같은 원리입니다. 발신자가 메시지나 트랜잭션에 서명하면, 공개 키를 가진 누구나 검증할 수 있습니다. 도구와 맥락은 다르지만, 핵심은 동일합니다. 중앙 기관 없이 암호학 키로 신뢰를 구축하는 것입니다.

### Why This Matters

- 비밀 키만이 유효한 서명을 생성할 수 있습니다.  
...
대부분의 사용자는 비밀 키나 암호학 연산을 직접 다루지 않습니다. 지갑이 백그라운드에서 키 관리와 트랜잭션 서명을 안전하게 처리합니다.
