...
* 트랜잭션, contract action, event를 인덱싱
...
* **Chain Indexer** - 노드에 연결하여 블록을 가져오고, 트랜잭션과 컨트랙트 활동을 추출해 데이터베이스에 기록합니다.
* **Wallet Indexer** - 활성 지갑을 모니터링하고 관련 event에 연결하여 개인화된 실시간 데이터 피드를 제공합니다.
* **Indexer API** - 인덱싱된 모든 데이터를 GraphQL로 제공하며 다음을 지원합니다:
  - 특정 쿼리(블록 높이/해시, tx ID, 컨트랙트 주소별)
  - 실시간 subscription(새 블록, 지갑 활동, 컨트랙트 호출)
  - 지갑별로 데이터 접근 범위를 제한하는 보안 세션

## Latest release v2.1.1 highlights

v2.1.1 릴리스에는 Indexer 관련 주요 패치와 내부 개선이 포함되어 있습니다. 트랜잭션 및 contract action 쿼리가 정확한 결과를 반환하도록 수정한 버그 픽스, Chain Indexer의 트랜잭션 적용 로직을 더 자연스러운 구조로 간소화한 리팩터링 등이 있습니다. Midnight Node v0.12, Midnight Wallet v5.0.0 이상, Lace Wallet v3.0.0 이상과 완벽히 호환됩니다.
