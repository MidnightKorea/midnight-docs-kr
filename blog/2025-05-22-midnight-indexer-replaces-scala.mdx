...
지난달 모듈형 고성능 indexing 서비스인 Midnight Indexer를 Rust로 작성하여 소개해 드렸습니다. 오늘은 이 Rust 기반 Midnight Indexer가 레거시 Scala 기반 Pub-Sub Indexer를 공식적으로 대체했다는 소식을 전합니다.
...
Midnight Indexer는 노드의 원시 데이터를 지갑과 애플리케이션이 조회할 수 있는 정보로 변환하는 고성능 모듈형 시스템입니다. 주요 기능은 다음과 같습니다:
...
v2.1.1 릴리스에는 Indexer에 대한 중요한 패치와 내부 개선 사항이 포함되어 있습니다. Transaction 및 contract action 쿼리가 정확한 결과를 반환하도록 수정하는 버그 픽스가 포함되어 있습니다. 또한 Chain Indexer에서 transaction 적용 로직을 더 관용적인 방식으로 간소화하는 리팩터링이 이루어졌습니다. 이 버전은 Midnight Node v0.12, Midnight Wallet v5.0.0 이상, Lace Wallet v3.0.0 이상과 완벽히 호환됩니다.
...
Scala 기반 Indexer는 더 이상 지원되지 않으며 추가 업데이트도 제공되지 않습니다. 모든 신규 개발, 도구, 지갑 통합은 이제 Rust 기반 Midnight Indexer에 맞춰 진행됩니다.
...
Rust 기반 Midnight Indexer가 오픈소스로 공개되었습니다. 소스 코드를 확인하고, 개선 사항을 기여하고, 이슈를 보고하거나, 프로젝트를 포크하여 자신만의 도구를 만들 수 있습니다. 모두 [GitHub](https://github.com/midnightntwrk/midnight-indexer)에서 직접 가능합니다.

모든 수준의 개발자 분들의 기여를 환영합니다. 버그 수정, 성능 개선, 기능 확장 등 어떤 형태든 좋습니다.
