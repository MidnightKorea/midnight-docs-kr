...
이 글에서는 컨트랙트의 핵심 기능을 다룹니다. 원장 상태 변수 관리, 민팅/소각/전송 등 주요 진입점과 서킷, 그리고 **Utils** 및 **Initializable** 모듈의 안전 및 유틸리티 함수를 살펴봅니다.
...
* **초기화 가드**: `Initializable_initialize`와 `Initializable_assertInitialized` 함수는 초기화 가드 역할을 합니다. 컨트랙트 상태가 단 한 번만 올바르게 설정되고, 초기화가 완료된 경우에만 이후 작업이 진행되도록 보장합니다. 상태를 읽거나 수정하는 모든 서킷은 `assert` 함수를 호출하여 초기화 무결성을 검증합니다.
