...
Midnight node는 이 Postgres 데이터베이스에서 Cardano 메인 체인 상태를 끊임없이 읽으면서 Midnight 블록을 생산하고, 파트너 체인 컨트랙트 이벤트를 읽고, NIGHT/DUST 생성 UTXO를 가져옵니다.
...
**핵심 정리:** 다운타임 최소화는 개별 노드의 가동률 지표를 지키는 일에 그치지 않습니다. Cardano가 그 아래에서 진화하는 동안 Midnight Network 전체의 건강, 처리량, 탈중앙화된 무결성을 지키는 일입니다.
