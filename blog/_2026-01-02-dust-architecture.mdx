...
Dust는 [Zswap](https://github.com/midnightntwrk/midnight-ledger/blob/main/spec/zswap.md)과 유사하지만 별도로 운영됩니다. Dust는 Midnight의 리소스 크레딧 시스템으로 기능합니다.
  ...
  - **shielded 및 양도 불가:** Dust는 가스 *전용*으로 쓰이는 shielded 용량 리소스입니다. 사용자 간 전송이 불가능합니다.
...
Zswap과 마찬가지로, Dust는 해시와 commitment/무효화자 패러다임 위에 구축됩니다. 각 Dust UTXO는 생성 시 추가 전용 머클 트리에 삽입되는 **commitment**와 소비 시 무효화자 집합에 삽입되는 **무효화자**를 가집니다.

Dust "소비"는 1대1 "전송"(자기 전송)입니다:

1.  **입력:** 1개의 Dust UTXO (무효화자).
2.  **출력:** 1개의 Dust UTXO (commitment).
...
개념적으로 Dust는 보유 중인 *Night* UTXO가 시간이 지남에 따라 생성합니다. 뒷받침하는 Night UTXO가 소비되지 않는 한, 연관된 Dust UTXO는 상한($\rho$)까지 가치를 생성합니다. 뒷받침하는 Night이 소비되면, Dust UTXO는 0으로 "소멸"됩니다.
...
**The Grace Period:**
Dust 사용은 shielded 상태이므로, 값은 *트랜잭션 생성* 시점으로 계산됩니다. 네트워크 지연을 고려하기 위해 프로토콜은 **Dust Grace Period**(예: 3시간)를 정의합니다. 트랜잭션의 타임스탬프가 블록 시간을 기준으로 이 범위 내에 있으면 수락됩니다.
...
상태 구성 요소에는 commitment 트리, 무효화자 집합, 루트 히스토리가 포함됩니다:
...
2.  시퀀스 번호($0, 1, 2...$)에 해당하는 commitment를 선형적으로 탐색합니다.
3.  **프라이버시:** 지갑은 인덱싱 서비스에 대한 프라이버시를 보호하기 위해 정확한 조회 대신 비트 접두사(확률적 필터링)를 사용하여 commitment를 쿼리해야 합니다.
...
7-9/ 블록 내 모든 이벤트가 일괄 처리되어 LedgerApi를 통해 단일 시스템 트랜잭션으로 래핑되고 Midnight ledger에서 실행됩니다.
