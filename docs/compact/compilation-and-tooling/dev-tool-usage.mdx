...
Compact 커맨드라인 도구의 사용법 페이지입니다. 이 도구로 Compact 툴체인의 컴파일러, 포매터, fixup 도구를 실행할 수 있습니다.
...
Compact 커맨드라인 도구는 Compact 스마트 컨트랙트 개발에 필요한 유틸리티 모음입니다.
...
Compact 커맨드라인 도구는 _command_를 인자로 받습니다. 이 command는 도구가 실행할 프로그램입니다. 지원되는 command 목록은 아래와 같으며, 복잡한 것들은 해당 command 이름 아래에서 상세히 설명합니다:

**check**

원격 서버에서 업데이트를 확인한 뒤 종료합니다 [aliases: ch].

**update**

Compact 툴체인을 최신 버전 또는 지정된 버전으로 업데이트한 뒤 종료합니다 [aliases: u, up].

**format**

Compact 파일을 포매팅한 뒤 종료합니다 [aliases: f, fmt].

**fixup**

Compact 파일에 fixup 변환을 적용한 뒤 종료합니다 [aliases: fx, fix].

**list**

사용 가능한 Compact 버전을 나열한 뒤 종료합니다 [aliases: l].

**clean**

설치된 모든 Compact 버전을 삭제한 뒤 종료합니다 [aliases: cl].
...
컴파일러를 호출한 뒤 종료합니다 [aliases: c].

**help**

커맨드라인 도구의 도움말을 출력한 뒤 종료합니다.

OPTIONS 
=======

아래 옵션은 모든 command에 적용되며, 지정 시 도구의 동작을 다음과 같이 변경합니다:
