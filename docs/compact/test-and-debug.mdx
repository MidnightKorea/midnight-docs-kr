...
* **Circuit logic testing**: 개별 circuit이 올바른 출력과 상태 전이를 만들어내는지 검증합니다.
...
* **Performance testing**: 증명 생성이 허용 가능한 시간 안에 완료되는지 확인합니다.
...
통합 테스트는 테스트 네트워크(Preview 또는 Preprod)에서 전체 트랜잭션 흐름을 검증합니다. Midnight 블록체인, proof server, 그 외 시스템 구성 요소와 컨트랙트가 올바르게 상호작용하는지 확인합니다.
...
프라이버시 테스트는 비공개 witness 데이터가 공개 트랜잭션 출력이나 블록체인 상태에 노출되지 않는지 확인합니다. 영지식 증명이 민감한 정보를 제대로 보호하는지 검증하는 핵심 단계입니다.

### Test disclosure behavior

명시적으로 공개된 값만 public state에 노출되는지 검증합니다. Compact의 `disclose()` 함수는 witness 값을 공개 대상으로 표시합니다.
...
전제 조건이 충족되지 않은 상황에서 circuit이 작업을 거부하는지 테스트합니다. Compact의 assert 구문은 런타임에 불변 조건을 강제합니다.
...
증명 생성이 실패하거나 시간 초과가 발생할 때는 다음 항목을 살펴봅니다.

- **Circuit complexity**: circuit 연산 한도가 합리적인지 확인합니다. 지나치게 복잡한 circuit은 proof server 용량 한계를 넘을 수 있습니다.
- **Infinite loops**: 종료되지 않을 가능성이 있는 루프를 확인합니다. 종료를 보장하려면 루프 카운터에 한도를 두세요.
- **Witness data types**: witness 함수가 circuit이 기대하는 형식의 데이터를 반환하는지 확인합니다. `Uint8Array` 값의 크기와 `bigint` 값의 유효 범위를 점검합니다.
