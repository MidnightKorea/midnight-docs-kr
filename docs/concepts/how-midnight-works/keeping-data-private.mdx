...
- [`persistentHash`](../reference/compact/compact-std-library/exports#persistentHash): 이진 데이터 해싱을 위한 기본 구성 요소
- [`persistentCommit`](../reference/compact/compact-std-library/exports#persistentCommit): 모든 Compact 유형에서 commitment를 생성하기 위한 원시형.
...
Compact 표준 라이브러리와 Compact JavaScript 대상 ADT는 이러한 연산을 위한 도구를 제공합니다. 구체적으로 [`MerkleTreePath<n, T>`](../reference/compact/compact-std-library/exports#merkletreepath) 유형, [`merkleTreePathRoot<n, T>()`](../reference/compact/compact-std-library/exports#merkle_tree_path_root) circuit, 그리고 [ledger 데이터 유형](../reference/compact/ledger-adt) 명세에 설명된 `MerkleTree`/`HistoricMerkleTree` JavaScript 상태 객체의 `pathForLeaf()`와 `findPathForLeaf()` 함수를 참조하세요.
