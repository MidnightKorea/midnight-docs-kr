...
Zswap[^1]은 Zerocash[^2]를 기반으로 한 shielded 토큰 메커니즘으로, *네이티브 토큰 지원*과 *원자적 스왑* 기능이 추가되었습니다. Zswap의 기본 구성 요소는 [offer](#offers)이며, 개념적으로 [input](#inputs)과 [output](#outputs)의 집합입니다. UTXO 모델과 유사하지만, 대응하는 input과 output을 서로 연결할 수 없다는 특성(Zerocash로부터 물려받은)으로 인해, 미사용 트랜잭션 집합 자체를 산출하는 것은 불가능합니다.
