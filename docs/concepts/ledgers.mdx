...
Midnight은 UTXO 모델 기반의 데이터 보호 blockchain입니다. 이 선택 덕분에 Midnight 고유의 프라이버시, 병렬 처리, 효율성이 실현됩니다. Midnight는 smart contract를 통해 account 스타일 token까지 지원합니다. 이 이중 접근 방식이 개발자에게 유연성을 제공하며, 이 모든 것은 두 가지 근본 모델을 이해하는 것에서 시작됩니다.
...
**Account 모델**은 Ethereum에서 익숙한 방식입니다. 각 행이 주소이고 잔액 열이 매 transaction마다 갱신되는 거대한 스프레드시트라고 보면 됩니다. 각 주소가 잔액이 포함된 영구 account를 가지고 있어, token을 전송하면 한쪽에서 빼고 다른 쪽에 더합니다.
...
애플리케이션마다 다른 요구가 있습니다. Midnight은 token에 두 가지 접근 방식을 모두 지원하여, 아키텍처 유연성을 제공합니다.
...
UTXO 모델의 구조는 병렬 transaction 처리를 자연스럽게 지원합니다. 각 UTXO가 독립적이므로, 서로 다른 UTXO를 사용하는 transaction은 동시에 처리할 수 있습니다. 예를 들어, Alice가 10개의 서로 다른 UTXO를 가지고 있다면 병렬 transaction으로 10명에게 동시에 전송할 수 있습니다.

반면, account 기반 시스템은 동일한 account에 영향을 미치는 transaction을 순차적으로 처리해야 합니다. 모든 transaction이 동일한 잔액을 수정하므로 자연스러운 병목이 생깁니다.
...
이 프라이버시 기능은 ledger token(UTXO 기반)과 contract token(account 기반) 모두에 적용되지만, 구현 방식이 다릅니다. Ledger token은 UTXO 수준에서 개별적으로 차폐할 수 있고, contract token은 Midnight의 비공개 smart contract 상태 모델을 활용합니다.
...
이 전환은 Midnight의 smart contract가 proof 생성과 함께 오프체인에서 실행된다는 점에서도 중요합니다. 모든 노드가 contract 코드를 재실행하는 대신, Midnight은 zero-knowledge circuit으로 올바른 실행을 증명합니다.

## Choosing the right token type

이러한 모델에 대한 이해는 Midnight에서 실제로 개발하기 위한 실용적 준비입니다. Ledger token과 contract token 사이의 선택은 가장 중요한 첫 번째 결정 중 하나가 될 것입니다.
