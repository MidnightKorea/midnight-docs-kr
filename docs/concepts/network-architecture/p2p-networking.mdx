...
피어를 발견하고 나면, 피어끼리 직접 연결을 맺고 공통으로 지원하는 프로토콜을 협상합니다. 이 단계에서 전송, 보안, 멀티플렉싱의 기반이 정해지며, 상위 계층의 동기화·gossip 프로토콜이 그 위에서 동작합니다.
...
* **WebSockets**: TCP 위에 구축되며, WebSocket 프레이밍을 통해 브라우저 호환 또는 프록시 호환 연결을 지원합니다.
...
피어는 Multistream-Select로 이 업그레이드를 협상하며, 이 과정에서 서로 호환되는 암호화·멀티플렉싱 옵션을 고릅니다.
