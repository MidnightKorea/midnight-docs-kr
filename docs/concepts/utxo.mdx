...
UTXO 모델은 디지털 가치에 대한 근본적으로 다른 사고방식입니다. Account 모델이 전통적인 은행 업무를 반영하여 익숙하게 느껴졌다면, UTXO 모델은 처음에는 낯설 수 있습니다. 이를 이해하면 설계상의 trade-off가 분명해지며, Midnight이 왜 프라이버시와 병렬 처리를 위한 기반으로 UTXO를 선택했는지 알 수 있습니다.

UTXO는 "Unspent 트랜잭션 Output"의 약자입니다. 각 UTXO는 지갑 속 물리적 현금처럼 개별적인 디지털 코인입니다. 각각 특정 값이 있고, 특정 소유자에게 속하며, 반드시 전체가 소비되어야 합니다. 이 제약이 UTXO 모델의 가장 유용한 속성을 뒷받침합니다.
...
$5 지폐를 찢어서 $3.50만 추출할 수는 없습니다. $5 지폐 전체를 건네고 $1.50의 거스름돈을 받아야 합니다. 원래 $5 지폐는 영원히 당신의 손을 떠나며 - 트랜잭션에서 "소비"된 것입니다 - 거스름돈으로 새 지폐를 받게 됩니다.
...
이는 단순한 설계 선택이 아니라 모델의 핵심 속성입니다. 각 UTXO는 완전한 단위로 처리됩니다:
...
분할할 수 없다는 이 성질 덕분에 원자적 트랜잭션, 병렬 처리, Midnight의 shielded 토큰 지원이 성립합니다.

### The nullifier set in Midnight

Midnight은 사용된 출력을 추적하는 방식에서 Bitcoin과 다릅니다. UTXO를 단순히 "사용됨"으로 표시하는 대신, Midnight은 nullifier 집합을 사용합니다:
...
모든 UTXO 트랜잭션은 원자적입니다. 입력이 전부 소비되고 출력이 전부 생성되거나, 아무 일도 일어나지 않거나 둘 중 하나입니다.
...
이는 사소한 최적화가 아닙니다. 고처리량 시나리오에서 이 아키텍처 차이는 처리량을 실질적으로 향상시킬 수 있습니다.
...
// shielded 및 unshielded 토큰 UTXO를 필요에 따라 사용 가능
```

Midnight에서 토큰 UTXO는 shielded 또는 unshielded가 될 수 있어, account별이 아닌 트랜잭션별로 프라이버시를 선택할 수 있습니다.
...
|--------------------------|-----------------------------|-------------------------------|----------------------------------------|
| 가치 저장            | 단일 변경 가능 잔액      | 불변 개별 코인      | shielded 토큰 구현 가능                |
| 트랜잭션 모델        | 상태 업데이트               | 상태 전환             | 자연스러운 감사 추적 및 증명 가능성    |
| 동시성              | 잠금 → 처리 → 해제     | 잠금 불필요               | 대규모 처리량 잠재력           |
| 프라이버시 접근         | 전체 잔액 혼합          | shielded 토큰 UTXO       | 세밀한 프라이버시 제어               |
...
* 변경 가능에서 불변으로: 잔액을 수정하는 대신 개별 값을 생성하고 소비합니다. 이를 통해 강력한 감사 가능성과 증명 시스템을 지원합니다.
