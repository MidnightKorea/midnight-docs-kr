...
- 명시적 상태 관리
- private state 데이터 검증
- circuit 접근 제어
- `List` 연산
- 중간 witness 기능

프론트엔드 테스트 스위트까지 갖춘 전체 DApp 예제는 [example-battleship](https://github.com/midnightntwrk/example-battleship)에서 확인하세요.
...
constructor(_x1: Uint<8>, _x2: Uint<8>) {
    ...
    // 모범 사례 예시 -- disclose(localSetBoard(_x1, _x2))를 사용하지 마세요,
...
export circuit checkBoard1(): [] {
    ...
    if(shotState == ShotState.MISS){
        ...
        // 신뢰하지 말고 검증하세요
        assert(board1.member(honestyCheckHash), "Cheat Detected: Player 1: claimed a HIT, when it was in fact a MISS. Why would they do that?");
