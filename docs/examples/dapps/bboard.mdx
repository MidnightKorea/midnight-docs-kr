...
이 규칙을 사용자의 신원을 보호하면서 온라인으로 구현하는 것이 과제입니다. 기존 시스템에서는 검증을 위해 서버에 신원 정보를 전송해야 합니다. Midnight는 더 나은 방법을 제공합니다. 사용자가 네트워크를 통해 개인 데이터를 보내지 않고도 영지식 증명을 통해 로컬에서 신원을 증명할 수 있습니다.
