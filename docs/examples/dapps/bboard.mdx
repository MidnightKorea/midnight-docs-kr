...
- Compact에서 private state를 가진 스마트 컨트랙트 작성
- 영지식 증명을 사용하여 신원을 공개하지 않고 검증
- 암호화 커밋 기반 접근 제어 구현

이 가이드를 마치면 게시판 컨트랙트가 프라이버시 규칙을 적용하는 방식과 CLI를 통해 컨트랙트를 배포하고 상호작용하는 방법을 파악할 수 있습니다.
...
게시판 컨트랙트는 Compact로 작성되었으며, 사용자가 신원을 공개하지 않고도 게시된 콘텐츠의 소유권을 증명할 수 있는 프라이버시 보호 애플리케이션의 구축 방법을 보여 줍니다.
...
세 개의 export circuit이 컨트랙트의 연산을 정의합니다:
...
컨트랙트는 하나의 witness 함수 `localSecretKey()`를 정의합니다. 이 함수는 circuit 실행 중에 사용자의 비밀 키를 온체인이나 생성된 증명에 노출하지 않고 반환합니다.
...
proof server는 개인 데이터 보호를 위해 트랜잭션의 영지식 증명을 로컬에서 생성합니다. 컨트랙트 배포나 상호작용 시 반드시 실행 중이어야 합니다.

다음 명령으로 proof server를 시작합니다:
...
:::tip
DApp을 사용하는 동안 proof server가 _반드시_ 실행 중이어야 합니다.
:::
...
게시판 CLI는 Lace Midnight Preview 같은 브라우저 지갑과는 별도로, 로컬에서 실행되는 헤드리스 지갑을 사용합니다.

#### Create a new wallet

메뉴에서 옵션 `1`을 선택하여 새 지갑을 만듭니다.

CLI가 새 지갑을 생성하고 정보를 표시합니다:

```
Initializing wallet builder for preprod
Your wallet seed is: <64자 wallet 시드> and your address is: <wallet 주소>
Using unshielded address: <unshielded 주소> waiting for funds...
...
wallet 시드를 안전한 곳에 저장하세요. 지갑 복구 시 시드가 필요합니다.
:::

#### Restore an existing wallet

이미 지갑 시드가 있다면 옵션 `2`를 선택하여 시드로 지갑을 복원합니다.

프롬프트가 나타나면 지갑 시드를 입력합니다:
...
CLI가 지갑을 복원하고 주소와 잔액을 표시합니다.

### Get faucet tokens

컨트랙트를 배포하려면 먼저 faucet에서 tNight 토큰을 받아야 합니다.

1. CLI 출력에서 unshielded 주소를 복사합니다.
2. [Preprod faucet](https://midnight-tmnight-preprod.nethermind.dev/)를 방문합니다.
3. unshielded 주소를 붙여넣습니다.
...
지갑이 tNight 보유량에서 컨트랙트 운영에 필요한 tDUST 네트워크 리소스를 자동으로 생성합니다.

### Deploy the contract

지갑에 자금이 충전되면 게시판 컨트랙트를 Preprod에 배포할 수 있습니다.
...
옵션 `4`를 선택하여 자신만 볼 수 있는 private state를 확인합니다:
...
CLI가 지갑을 정상 종료하고 테스트 환경을 중지합니다.
