...
[`create-mn-app`](https://www.npmjs.com/package/create-mn-app) CLI 도구는 Midnight DApp용 스타터 템플릿을 제공합니다. TypeScript 설정, 핫 리로딩, wallet 운영 로직이 사전 구성되어 있습니다.
...
CLI가 템플릿을 선택하라는 프롬프트를 표시합니다. 사용 가능한 옵션:
...
CLI가 새 wallet 생성 또는 기존 wallet 복원을 선택하라는 프롬프트를 표시합니다. 새 wallet을 생성하면 CLI가 wallet seed를 생성하고 wallet 주소를 표시합니다. Wallet seed는 복원 시 필요하므로 안전한 곳에 저장하세요.
...
ZK proof를 사용하여 온체인에 신원을 공개하지 않으면서 게시자와 메시지 소유자의 신원을 검증합니다. 자세한 내용은 [Bulletin board DApp](../examples/dapps/bboard) 예제를 참고하세요.
