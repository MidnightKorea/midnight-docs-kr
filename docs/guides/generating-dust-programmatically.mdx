---
title: Preprod에서 프로그래밍 방식으로 DUST 생성하기
...
이 튜토리얼은 전체 과정을 처음부터 안내합니다: 빈 프로젝트 설정, 지갑 생성 또는 복원, tNIGHT 충전, 그리고 DUST 생성을 위한 토큰 등록까지. 완료하면 지갑을 설정하고 DUST 생성을 시작하는 TypeScript 스크립트를 갖게 됩니다.
...
Midnight는 데이터 프라이버시를 위해 영지식 증명을 사용합니다. Proof server는 이 증명을 생성하는 서비스이며, 로컬 머신에서 실행됩니다. 프라이빗 데이터가 컴퓨터 밖으로 나가지 않도록 보장합니다. Docker를 사용하면 간단하게 proof server를 실행할 수 있습니다: 하나의 명령어로 이미지를 풀하고 시작합니다.
...
모든 Node.js 프로젝트에는 의존성과 스크립트를 선언하는 `package.json` 파일이 필요합니다. 파일을 생성합니다.
...
Docker에 proof server 실행 방법을 알려주는 설정입니다. 공식 Midnight proof server 이미지를 다운로드하고 포트 6300으로 노출하여 스크립트가 `http://localhost:6300`으로 증명 요청을 보낼 수 있게 합니다.
...
스크립트에는 여러 패키지가 필요합니다: WebSocket polyfill(Node.js에 내장되어 있지 않으므로), shielded/unshielded/dust 지갑용 Midnight wallet SDK 패키지, HD 키 파생 라이브러리, 주소 포매팅 유틸리티 등입니다. 먼저 전역 WebSocket을 설정하여 wallet SDK가 indexer에 연결할 수 있게 합니다.
...
이 스크립트의 여러 작업(지갑 빌드, 네트워크 동기화, NIGHT 등록)은 완료까지 몇 초가 걸립니다. 사용자에게 시각적 피드백을 주기 위해, 장시간 실행되는 각 작업을 스피너 애니메이션과 함께 표시하는 헬퍼로 감쌉니다.
...
스크립트는 사용자에게 몇 가지를 대화형으로 질문해야 합니다(지갑 생성/복원 여부, 지정할 dust 주소 등). Node의 내장 `readline` 모듈을 Promise로 감싸는 작은 `prompt` 헬퍼를 사용하여 사용자 입력을 `await`할 수 있게 합니다.
...
DUST 생성을 위해 dust 주소를 지정할 때, 실수로 shielded나 unshielded 주소를 붙여넣지 않았는지 확인하는 것이 중요합니다. 이 헬퍼는 bech32m 형식을 검증하고, 사용자가 유효한 값을 입력할 때까지(또는 Enter를 눌러 자신의 dust 주소를 사용할 때까지) 반복합니다.
...
모든 Midnight 지갑은 32바이트 시드에서 파생됩니다. 이 함수는 사용자에게 새 지갑 생성(새로운 랜덤 시드 생성)과 기존 지갑 복원(기존 시드 입력) 중 하나를 선택하게 합니다. 어느 경우든 시드를 hex 문자열로 반환합니다.
...
하나의 시드에서 HD(hierarchical deterministic) 파생을 통해 세 세트의 키가 생성됩니다: shielded 트랜잭션용, unshielded 트랜잭션용, DUST용입니다. 이 함수는 HD wallet 라이브러리에 세 키를 한꺼번에 파생하도록 요청하고, 이후 민감한 키 데이터를 메모리에서 삭제합니다.
...
세 개의 서브 지갑을 하나로 결합하는 단계입니다. 각 서브 지갑은 고유한 설정으로 생성된 뒤, `WalletFacade.init()`에 전달되어 하나의 통합 인터페이스가 됩니다. 이 facade가 나머지 스크립트에서 사용하는 대상이며, 내부적으로 세 지갑이 함께 동작한다는 사실을 추상화합니다.
...
지갑이 빌드되면 사용하기 전에 네트워크와 동기화해야 합니다. 지갑은 RxJS observable로 상태 업데이트를 방출하므로, `isSynced`가 true인 첫 번째 상태를 기다립니다.
...
스크립트의 핵심 부분입니다. NIGHT 토큰은 자동으로 DUST를 생성하지 않으며, 온체인 트랜잭션을 통해 명시적으로 등록해야 합니다. 이 함수는 아직 등록되지 않은 NIGHT UTXO를 찾아 등록 트랜잭션을 구성하고, unshielded keystore로 서명한 뒤 제출합니다. 선택적 `targetDustAddress` 매개변수로 생성된 DUST를 다른 지갑으로 보낼 수 있습니다(지정하지 않으면 스크립트 자체의 dust 주소로 전송).
...
DUST는 시간이 지남에 따라 지속적으로 쌓이므로, 현재 잔액을 수시로 확인하면 유용합니다. 최신 동기화 상태를 가져와 현재 시점의 dust 잔액을 반환하는 간단한 헬퍼입니다.
...
스크립트를 준비했으니, 스크립트가 요청하는 영지식 증명을 처리할 수 있도록 proof server를 시작합니다. 두 번째 터미널 창을 열고, 프로젝트 폴더로 이동한 뒤 proof server를 시작합니다:
...
처음 실행 시 Docker가 proof server 이미지(~2 GB)를 다운로드합니다. 출력에 `listening on: 0.0.0.0:6300`이 나타나면 proof server가 준비된 것입니다.
...
2. 브라우저에서 faucet 링크를 엽니다: [https://faucet.preprod.midnight.network](https://faucet.preprod.midnight.network)
...
**포트 6300 연결 거부**: proof server가 실행되지 않고 있습니다. 다른 터미널에서 `docker compose -f proof-server.yml up`이 실행 중이고 `listening on: 0.0.0.0:6300`이 표시되는지 확인하세요.
...
DUST 생성이 활성화되면 지갑이 트랜잭션 수수료를 지불할 준비가 된 것입니다. 여기서부터 [Midnight에 스마트 컨트랙트 배포하기](https://docs.midnight.network/guides/deploy-mn-app)를 탐색하거나, 자체 애플리케이션의 지갑 설정 흐름에 DUST 생성을 통합할 수 있습니다. [Awesome DApps 리포지토리](https://github.com/midnightntwrk/awesome-dapps)와 [Midnight 문서](https://docs.midnight.network/)에서 더 많은 정보를 확인하세요.
