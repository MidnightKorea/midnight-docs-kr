...
각 circuit은 `contract.circuits` 또는 `contract.impureCircuits`에 JavaScript 함수로 노출됩니다. 이 래퍼가 입력 준비, JavaScript 구현 실행을 처리하고, 출력, 업데이트된 컨텍스트, 증명 데이터가 포함된 결과를 반환합니다.
...
- `proofData`: 증명 생성을 위한 입력, 출력, 트랜스크립트 데이터
- `gasCost`: 가스 비용 추적 정보
...
JavaScript 구현은 저수준 증명 시스템과 고수준 컨트랙트 로직 사이의 다리 역할을 합니다. `contract.circuits.post(context, "Hello world!")`를 호출하면 ZK circuit이 온체인에서 실행하는 것과 동일한 로직이 실행되지만, Node.js에서 단계별로 실행하고 로그를 남기고 검사할 수 있습니다.

따라서 증명을 생성하거나 트랜잭션을 제출하기 전에 로컬에서 컨트랙트 동작을 미리 검증할 수 있습니다.
