...
Mainnet 데이터베이스 snapshot을 사용한 경우, 노드가 내부 초기화를 마치기 전까지는 `node.socket` 파일이 생성되지 *않습니다*. 이 과정에는 데이터베이스 검증과 필요한 block의 replay가 포함됩니다. 따라서 잠시 기다려야 합니다.
...
`cardano-node`의 최신 block 높이를 확인하세요.
...
- **`"block"`**: 노드가 마지막으로 처리한 block의 높이입니다. 여기서는 block *136,111*에 도달했습니다.
...
- **`"hash"`**: 노드가 처리한 가장 최근 block의 고유한 디지털 지문입니다.
