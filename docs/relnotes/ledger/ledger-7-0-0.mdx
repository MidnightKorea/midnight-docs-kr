...
description: Ledger는 Midnight 네트워크와 상호작용하여 동기화, 트랜잭션 검증, 체인 상태 유지 등을 수행하는 블록체인 인프라 컴포넌트입니다.
...
ZKIR WASM 바인딩에서 circuit k-값 계산 기능을 추가했습니다. ZKIR v2 프로토콜 circuit 테스트도 포함되어 있습니다.
...
ledger가 공식 Midnight SRS와 midnight-zk 1.0을 사용합니다.
...
ledger에서는 블록이 비어 있다고 판단하고 노드에서는 가득 찼다고 판단하여 가격이 하락하던 불일치 문제를 해결합니다.
...
| PM-21160 | WASM의 proof provider 래퍼가 `undefined` 값을 잘못 처리하는 문제를 수정했습니다. |
...
| PR-39 | proof server가 Dust 키를 잘못 가져오는 문제를 수정했습니다. |
