...
Indexer API를 v3으로 업데이트하며, Ledger v7과 노드 0.20을 완전 지원합니다. 호환성을 깨는 변경이 포함되어 모든 사용자에게 영향을 줍니다.
...
- 포괄적인 unshielded(unshielded) 토큰 기능
...
Indexer가 DUST 및 cNIGHT를 포함한 unshielded 토큰을 완전히 지원합니다.
unshielded 토큰은 컨트랙트가 소유할 수 있으며, 구독에 동기화 진행 데이터가 포함됩니다.
...
시스템 트랜잭션도 지원하며, `ApplyStage`가 세그먼트 결과를 포함하는 `TransactionResult`로 이름이 변경되었습니다.

### API improvements

클라이언트/서버 오류 타입을 포함하는 `ApiError`가 추가되었습니다.
...
**변경 내용**: 노드 0.12 지원이 제거되었으며, Node v0.20 및 Ledger v7만 지원합니다.

**영향**: 이전 Ledger 또는 노드 버전을 사용하는 DApp은 Indexer v3에서 동작하지 않습니다.

**필요한 조치**: Ledger를 v7로, 노드를 v0.20.0으로 업데이트하세요.
