...
description: Midnight.js는 Midnight 블록체인에서 탈중앙화 애플리케이션 개발을 지원하는 클라이언트 라이브러리입니다.
...
- unshielded(unshielded) NIGHT 토큰 잔액 조회 지원
- 트랜잭션 TTL(유효 기간) 지원
...
공개 indexer를 통해 unshielded NIGHT 토큰 잔액을 조회할 수 있습니다.
...
성능 향상을 위해 바이너리(Uint8Array) circuit 결과를 도입했습니다:
- circuit 실행 중 메모리 할당 감소
- 더 빠른 직렬화 및 역직렬화
- 대규모 또는 복잡한 circuit 작업 시 성능 향상
