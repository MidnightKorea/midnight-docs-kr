...
- 지갑 연동 DApp Connector API를 통한 proving을 구현하는 경우
...
DApp Connector의 proving API를 Midnight.js `ProofProvider` 인터페이스에 맞춰 변환하는 패키지로, 독립형 proof server 없이 지갑에 proving을 위임할 수 있습니다.
...
이 변경으로 모든 output을 지갑 자체 키로 암호화하여 발생하던 phantom balance 문제가 방지됩니다.
...
Compact fetch가 `GITHUB_TOKEN`을 지원하여, 더 높은 rate limit과 private release 접근을 위해 인증된 GitHub API를 사용할 수 있습니다.
