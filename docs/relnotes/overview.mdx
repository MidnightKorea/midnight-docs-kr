...
이번 릴리스는 Preview, Preprod, Mainnet 환경에 Midnight Ledger 8.0을 제공하며, 런타임, SDK, 도구 업데이트를 포함합니다.
...
아래는 DApp 개발을 위한 Preview, Preprod, Mainnet 환경별 유용한 링크입니다.
...
아래 호환성 매트릭스는 이번 릴리스 기준으로 Preview, Preprod, Mainnet 환경에서 함께 테스트된 버전을 나타냅니다.
...
아래 목록은 DApp 개발에 필요한 각 컴포넌트의 상세 릴리스 노트를 담고 있습니다.
