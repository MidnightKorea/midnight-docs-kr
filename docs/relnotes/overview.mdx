...
Midnight은 RPC 노드, 인덱서 API, proof server 등 전용 네트워크 서비스를 통해 Preview, Preprod, Mainnet 환경에서 DApp 개발과 상호작용을 지원합니다. 애플리케이션 연결과 서비스 설정에 대한 자세한 내용은 [Environments and endpoints](./network)를 참고하세요.
