...
여기서 `wallet-sdk` 버전이 중요합니다. [호환성 매트릭스](/relnotes/support-matrix)에 명시된 버전인 1.2.0으로 정확히 고정하세요. npm의 `latest` dist-tag는 아직 1.1.0을 가리키므로, 버전을 정확히 지정하지 않고 설치하면 조용히 더 낮은 릴리스로 해석됩니다.
