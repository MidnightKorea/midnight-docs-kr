...
Midnight에서는 토큰에 어떤 프라이버시 모델을 선택하느냐에 따라 smart contract를 작성하는 방식이 달라집니다. 이 튜토리얼에서는 Compact로 **unshielded** 토큰, 즉 잔액이 온체인에 남아 누구나 읽을 수 있는 투명한 토큰을 만들고 전송해 봅니다.
...
이 튜토리얼은 Compact 기본기를 알고 smart contract를 배포해 본 개발자를 대상으로 합니다. Compact가 처음이라면 [Compact 언어 레퍼런스](/compact/reference/compact-reference)를 먼저 읽거나 [Midnight Academy](https://academy.midnight.network)를 살펴보세요.

## How tokens work on Midnight

Midnight에는 프로토콜이 직접 발행하는 네이티브 토큰 NIGHT 하나가 있습니다. 테스트 네트워크에서는 faucet에서 tNIGHT으로 받거나 로컬 네트워크의 genesis wallet에 미리 충전된 상태로 받습니다. 메인넷에서는 Cardano에서 단방향 cNIGHT → mNIGHT 브리지를 통해 Midnight으로 넘어옵니다. 전체 그림은 [토큰 개요](/tokens/overview)를 참고하세요. NIGHT 외의 *나머지* 토큰은 모두 smart contract가 만들며, 이 문서에서 하는 작업이 바로 그것입니다.
...
| **컨트랙트** | smart contract가 mint, 잔액은 온체인 | smart contract가 mint, 잔액 비공개 |

2부작 튜토리얼은 이 표의 **컨트랙트** 행 두 개를 다룹니다. 이 문서가 unshielded 열이고, [shielded 튜토리얼](/tokens/shielded-token)이 shielded 열입니다. 네이티브 토큰은 프로토콜이 발행하며, 아래에서 자금 조달에 쓰는 NIGHT이 여기에 해당합니다. 세 번째 갈래인 hybrid 토큰은 네이티브 코인을 smart contract의 통제 아래 두는 방식인데, 아직 설계 중인 프로토콜 변경에 의존하므로 이 문서의 범위 밖입니다.

:::info[토큰 표준]

Midnight의 토큰 표준은 아직 정리되는 중입니다. 이 튜토리얼의 smart contract는 초안 단계의 표준 대신 지금 동작하는 프로토콜 primitive를 직접 사용합니다. 현재 논의 방향은 [토큰 표준 논의 스레드](https://github.com/midnightntwrk/midnight-improvement-proposals/discussions/142)를 참고하세요.
...
모든 토큰에는 토큰 타입을 식별하는 `Bytes<32>` 값인 *color*가 있습니다. 컨트랙트 토큰의 color는 smart contract와 여러분이 정하는 고정 라벨인 *domain separator*에서 나옵니다. mint primitive가 color를 반환하며, 이후 그 토큰에 대한 모든 연산에 이 color를 씁니다. color가 smart contract에 의존하기 때문에, 같은 라벨을 쓰는 두 smart contract도 서로 다른 토큰을 만듭니다.
...
이 문서의 핵심 개념입니다. 프라이버시 모델은 마지막에 켜고 끄는 플래그가 아니라, smart contract가 무엇을 저장해야 하는지를 바꿉니다.

**unshielded** 쪽에서는 체인이 주소별로 각 토큰 color의 잔액을 기록합니다. smart contract는 `unshieldedBalance(color)`로 자신의 잔액을 읽습니다. 잔액과 총 공급량이 공개되므로 `balanceOf`와 `totalSupply`에 해당하는 투명한 구현이 성립합니다.

**shielded** 쪽에서는 체인이 잔액을 감춥니다. 금액이 아니라 commitment와 nullifier를 기록합니다. 읽을 수 있는 공개 잔액 자체가 없습니다. 누가 무엇을 보유하는지 알아야 하는 smart contract는 이를 직접 추적해야 하고, 그 값을 shielded *coin*으로 관리해야 합니다. 그래서 shielded smart contract는 잔액을 전혀 추적하지 않고 하나의 transaction 안에서 coin을 mint해 호출자에게 전달하는 반면, unshielded 쪽은 체인의 공개 잔액에 기댑니다.

|  | Unshielded 토큰 | Shielded 토큰 |
| :---- | :---- | :---- |
| 잔액 저장 | 온체인, 주소별 | 비공개, smart contract가 추적 |
| 잔액 읽기 | `unshieldedBalance(color)` | 공개 조회 불가, smart contract 상태만 |
| 컨트랙트가 보유하는 값 | 잔액 | 변경 불가능한 coin |

shielded coin은 변경할 수 없습니다. 값을 제자리에서 바꿀 수 없습니다. 금액을 바꾸려면 smart contract가 현재 coin을 소비하고 새 금액의 coin을 새로 만들어야 합니다. nonce가 각 coin을 고유하게 유지합니다. 이 소비 후 재생성 패턴은 shielded 튜토리얼의 [Go further](/tokens/shielded-token#go-further-with-committed-transfers-and-burns) 섹션에서 확인할 수 있습니다.
...
1. **wallet 대 wallet:** 보유자가 wallet에 토큰을 갖고 있다면 wallet이 직접 전송합니다. smart contract는 관여하지 않습니다.
2. **컨트랙트 경유:** smart contract가 자신이 보유한 토큰을 옮깁니다. 투명한 토큰은 `sendUnshielded`로, shielded 토큰은 shielded coin을 보내는 방식으로 처리합니다.

shielded 토큰에서는 이 점을 분명히 짚어야 합니다. smart contract는 coin을 직접 보관하는 동안에만 shielded 값에 규칙을 강제할 수 있습니다. 사용자가 coin을 자기 wallet에 갖는 순간부터는 smart contract 바깥에서 자기 키로 사용합니다. shielded 토큰을 통제해야 하는 smart contract가 coin을 볼트에 직접 보관하는 이유가 여기에 있습니다.
...
이 튜토리얼은 로컬 Midnight 네트워크에서 실행합니다. 로컬 네트워크에는 NIGHT이 충전되어 있고 수수료 등록까지 끝난 genesis wallet이 있어서, faucet 없이 몇 초 만에 배포할 수 있습니다. [midnight-local-dev](https://github.com/midnightntwrk/midnight-local-dev.git) 도구가 노드, indexer, proof server를 Docker로 띄우고 이 wallet을 초기화해 줍니다.
...
그리고 Preprod faucet에서 직접 wallet에 자금을 채웁니다([Run it](#run-it)에서 다룹니다). 흐름은 같고, 로컬보다 몇 분 더 걸릴 뿐입니다.
...
- 첫 `assert`가 권한 게이트입니다. `sk`로 owner 키를 다시 계산해 저장된 `owner`와 일치하는지 확인하므로 배포자만 mint할 수 있습니다. `sk`는 평범한 circuit 파라미터라 비공개로 유지되고, 비교는 proof 안에서 일어나므로 이 assert에는 `disclose()`가 필요 없습니다.
...
컨트랙트에서 wallet으로 토큰을 옮기는 transfer circuit을 추가합니다.
...
그다음 `sendUnshielded(...)`가 해당 금액을 수신자에게 보냅니다. 이번에는 수신자를 `right<ContractAddress, UserAddress>(...)`로 감싸 주소 타입의 `UserAddress` 쪽임을 표시합니다. 토큰이 컨트랙트가 아니라 wallet으로 가기 때문입니다.

### Read the balance

unshielded 잔액을 읽는 데는 circuit이 필요 없습니다. 토큰이 투명하므로 잔액이 온체인에 공개되어 있고, Midnight.js가 wallet 상태에서 바로 읽어 옵니다. `getUnshieldedBalances`가 토큰 color에서 금액으로의 맵을 반환합니다(내부적으로는 `unshielded.balances`입니다). 다음 섹션의 배포 스크립트도 정확히 이 방식을 써서, 컨트랙트를 호출하지 않고 `after.unshielded.balances[color]`를 읽어 전송 결과를 확인합니다.
...
컴파일러는 컨트랙트의 TypeScript 인터페이스, prover 키와 verifier 키, proof server가 사용하는 ZK 산출물을 모두 `src/managed/unshielded-token` 아래에 생성합니다. 다음 섹션의 배포 스크립트는 `src/managed/unshielded-token/contract/index.js`에서 생성된 컨트랙트를 import해 배포용으로 감싸고, `ledger`로 이 토큰의 공개 상태 `token_color`, `initialized`, `owner`를 디코딩합니다. 컴파일이 끝났으니 이제 배포하고 첫 토큰을 mint할 차례입니다.

## Deploy and test your token

이제 실제 네트워크에서 컨트랙트를 실행합니다. wallet 패키지를 추가하고, 코드를 네트워크에 연결하는 작은 지원 파일 네 개를 만든 다음, 토큰을 만들고 다시 읽고 wallet으로 전송하는 배포 스크립트를 작성합니다.

아래 단계는 기본적으로 **로컬** 네트워크를 사용합니다. **Preprod**에서 실행하려면 스크립트 실행 시 `MIDNIGHT_NETWORK=preprod`를 설정하세요. 나머지는 그대로입니다.

:::info

이 작업을 하는 동안 로컬 네트워크(Preprod라면 독립 proof server)를 계속 실행해 두세요. 배포와 모든 circuit 호출은 proof server를 통해 zero-knowledge proof를 생성합니다.
...
지원 파일 중 보일러플레이트가 가장 많은 파일입니다. wallet-sdk 파사드 위에 wallet을 구성하고, 컨트랙트 프레임워크가 요구하는 `WalletProvider`와 `MidnightProvider` 인터페이스에 맞춰 어댑팅합니다. 그대로 복사해 쓰세요. shielded 튜토리얼에서도 수정 없이 재사용합니다.
...
배포 스크립트에서 호출하는 함수는 두 개입니다. seed로 wallet을 만드는 `MidnightWalletProvider.build(...)`, 그리고 wallet이 체인을 따라잡을 때까지 기다렸다가 현재 상태를 반환하는 `syncWallet(...)`입니다.
...
프레임워크가 사용하는 provider 세트를 조립합니다. 공개 상태는 indexer에서, proof는 proof server에서, ZK 설정은 컴파일 산출물에서 가져오고, 여기에 방금 만든 wallet을 더합니다.
...
seed로 wallet을 만들고 시작한 뒤, 자금을 채울 수 있도록 주소를 출력합니다.
...
wallet seed는 `MIDNIGHT_SEED`로 넘깁니다. 스크립트는 wallet을 만들고 unshielded 주소를 출력한 뒤, 그 주소에 NIGHT이 들어올 때까지 기다립니다. 로컬에서는 genesis wallet에 이미 자금이 있으므로 대기 없이 바로 넘어갑니다. Preprod에서는 스크립트가 기다리는 동안 출력된 주소로 faucet에서 자금을 채우면 됩니다.
...
이 필터는 등록 플래그뿐 아니라 토큰 타입도 확인합니다. DUST 생성 등록은 NIGHT에만 가능하고, 다른 토큰 타입이 하나라도 섞이면 wallet SDK가 호출 전체를 거부합니다. 타입 확인은 재실행 시 특히 중요합니다. 아래에서 mint한 토큰이 wallet에 들어오면 그 UTXO들도 `registeredForDustGeneration === false`로 보고되는데, `nightRaw` 검사가 없으면 스크립트가 이들까지 등록 호출에 넘겨 실패하게 됩니다.

이어지는 transaction에 수수료를 낼 수 있도록 DUST가 생길 때까지 폴링합니다.
...
`deployContract`는 온체인 주소를 반환하고, `args`는 constructor 인자를 전달합니다. 여기서는 owner secret입니다. constructor는 secret의 해시만 저장하므로 secret 자체는 스크립트를 벗어나지 않습니다. `queryContractState`는 컨트랙트의 공개 상태를 가져오고, `ledger(...)`가 이를 여러분이 선언한 필드로 디코딩합니다. 배포 직후 `initialized`는 false입니다.
...
`submitCallTx`가 mint circuit을 온체인에서 실행합니다. `args`는 Compact 시그니처와 같은 순서로, owner secret이 먼저 오고 금액이 뒤따릅니다. secret은 witness 데이터이므로 transaction에 드러나지 않고 proof에만 입력됩니다.

실행 후 `initialized`는 true가 되고 `token_color`에는 mint가 만든 color가 담깁니다. 이제 토큰이 존재하며, 1,000단위 전부를 컨트랙트가 보유합니다.

일부를 자신의 wallet으로 전송한 뒤 wallet 잔액을 읽어 도착했는지 확인합니다.
...
UserAddress circuit 인자는 수신자의 원시 주소 바이트를 담은 `{ bytes: Uint8Array }` 형태의 객체입니다. 그래서 `toUserAddressBytes`가 wallet 주소에서 그 바이트를 꺼냅니다. 이어지는 transfer 호출이 컨트랙트에서 여러분의 wallet 주소로 100단위를 옮깁니다. 다시 sync하면 wallet이 해당 토큰 color에 대해 잔액 100을 보고하며, 이것이 전송이 온체인에 반영되었다는 증거입니다.
...
같은 스크립트를 Preprod에서 실행할 때 달라지는 점은 두 가지입니다. 자금이 있는 wallet을 직접 준비해야 하고, 독립 proof server를 계속 실행해 두어야 합니다([Start the local network](#start-the-local-network) 참고).
...
이 seed는 일회용 테스트 wallet으로만 다루세요. 테스트 자금만 담기며, 실제 용도로 재사용해서는 안 됩니다. 그다음 원격 sync가 더 오래 걸리므로 Node에 메모리를 넉넉히 주고 그 seed로 스크립트를 실행합니다.
...
스크립트가 wallet 주소를 출력하고 대기합니다. 그 주소를 복사해 [Preprod faucet](https://midnight-tmnight-preprod.nethermind.dev/)에 붙여 넣고 tNIGHT을 요청하세요. 자금이 들어오면 스크립트가 DUST 생성을 위해 등록하고, DUST가 생길 때까지 기다린 뒤 로컬과 똑같이 배포하고, mint하고, 전송합니다.

로컬 실행보다 몇 분 정도 더 걸린다고 보면 됩니다.

## What's next

다음은 [이 토큰의 shielded 버전](/tokens/shielded-token)을 만들 차례입니다. shielded 잔액은 감춰져 있어 체인이 대신 추적해 주지 못하므로, 컨트랙트가 하나의 transaction 안에서 shielded coin을 mint해 호출자에게 전달하고, 여러분은 wallet의 shielded 잔액을 읽어 결과를 확인합니다.

이 문서에서 만든 wallet과 provider 설정을 그대로 재사용하며, 접속하는 네트워크도 같습니다.
