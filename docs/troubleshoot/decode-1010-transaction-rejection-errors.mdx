...
아래의 [variant 테이블](#look-up-the-variant)에서 `u8` 값을 찾아 해당하는 원장 오류 이름과 분류를 확인하세요.
...
증명 서버가 노드 및 SDK와 호환되는 버전으로 실행 중인지 확인하세요. [proof server 실행](../guides/run-proof-server.mdx)과 [릴리스 호환성 매트릭스](../relnotes/support-matrix.mdx)를 참조하세요.
...
응답에 `code: 1010`은 있지만 `Custom error: N` 부분이 없다면, Midnight 원장 로직이 아니라 그 위 단계인 Substrate 검증에서 거부가 일어난 것입니다. 가장 흔한 원인은 다음과 같습니다.
...
- **Check the compatibility matrix**: 증명 서버, 런타임, SDK가 [릴리스 호환성 매트릭스](../relnotes/support-matrix.mdx)에서 지원하는 조합과 일치하는지 확인하세요.
