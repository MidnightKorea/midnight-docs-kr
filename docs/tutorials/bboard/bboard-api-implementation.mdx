...
프로덕션 빌드에서 테스트 파일을 제외하도록 `api/tsconfig.build.json`을 만드세요:
...
`post` 메서드는 스마트 컨트랙트의 `post` circuit을 호출하고, ZK proof가 생성되고 트랜잭션이 확인될 때까지 대기한 후 트랜잭션 세부 정보를 로깅합니다.
...
`takeDown` 메서드는 스마트 컨트랙트의 `takeDown` circuit을 호출하며, 소유권 증명이 필요합니다. 호출자의 해싱된 비밀 키가 온체인 소유자 필드와 일치하는 경우에만 성공할 수 있습니다.
...
`deploy` 메서드는 배포자의 private 상태에 사용할 새로운 32바이트 비밀 키를 생성하여 블록체인에 새로운 게시판 컨트랙트 인스턴스를 만듭니다. 배포된 각 컨트랙트는 다른 사용자가 게시판에 참여하고 상호작용할 수 있도록 고유한 온체인 주소를 받습니다.
...
`join` 메서드는 기존에 배포된 게시판 컨트랙트에 연결합니다. 연결 후 public state를 읽을 수 있으며, 현재 메시지의 게시자라면 비밀 키를 사용하여 메시지를 제거할 수 있습니다.
...
Midnight.js 4.x의 private 상태 provider는 저장된 상태를 컨트랙트 주소별로 구분하므로, 이 헬퍼는 상태를 읽기 전에 먼저 `setContractAddress()`를 호출합니다. 해당 주소에 대한 private 상태가 없으면 새로운 32바이트 난수 비밀 키를 생성합니다. private 상태는 애플리케이션 재시작 시에도 유지되어, 사용자가 향후 세션에서 자신이 게시한 메시지의 소유권을 proof할 수 있습니다.
