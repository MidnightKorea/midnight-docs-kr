...
컨트랙트를 배포하는 것이 논리적으로 첫 번째 작업입니다. 컨트랙트 코드를 네트워크에 전송하고 MidnightJS 호출을 통해 컨트랙트 회로에 접근할 수 있게 합니다. MidnightJS의 `deployContract` 함수를 호출하려면 *배포자 Alice*를 위한 초기 private state를 생성해야 합니다:
```typescript
    it('deploys the contract', async () => {
