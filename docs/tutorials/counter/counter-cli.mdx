 ...
 *
 * Preprod/Standalone에서 NIGHT 토큰은 시간이 지남에 따라 DUST를 생성하지만,
...
```typescript title="counter-cli/src/api.ts"
/**
 * 16진수 시드에서 지갑을 빌드(또는 복원)한 후, 지갑이 동기화되고
 * 자금을 수령할 때까지 대기한 다음 반환합니다.
 *
 * 단계:
 *   1. 시드에서 HD 키(Zswap, NightExternal, Dust)를 파생합니다.
 *   2. 세 개의 서브 지갑(Shielded, Unshielded, Dust)을 생성합니다.
 *   3. WalletFacade를 시작하고 동기화를 대기합니다.
 *   4. 모든 주소가 포함된 지갑 요약을 표시합니다.
 *   5. 잔액이 0이면 입금될 자금을 대기합니다(예: faucet에서).
 */
export const buildWalletAndWaitForFunds = async (config: Config, seed: string): Promise<WalletContext> => {
  ...
  // 동기화 중 사용자가 faucet으로 자금을 조달할 수 있도록 비차폐 주소를 즉시 표시
  const networkId = getNetworkId();
  const DIV = '──────────────────────────────────────────────────────────────';
  console.log(`
${DIV}
...
`);
  ...
  // Preprod/Standalone에서 tx 수수료를 위해 NIGHT UTXO를 dust 생성에 등록 (필수)
...
5. **dust 생성 등록**: 지갑의 NIGHT UTXO를 dust 생성 서비스에 등록하는 단계로, Preprod 및 Standalone 네트워크에서 트랜잭션 수수료에 필요합니다.
...
export const configureProviders = async (ctx: WalletContext, config: Config) => {
  ...
  // accountId와 privateStoragePasswordProvider는 levelPrivateStateProvider에 필요합니다.
  // 비밀번호로는 coin 공개 키를 base64로 인코딩해 사용합니다. base64 출력은 네 가지
  // 문자 클래스를 모두 포함하며, 원시 16진수 문자열에서 나타나는 반복 문자 연속을 피합니다.
    ...
    switch (choice.trim()) {
      case '1':
        try {
          ...
          console.log(`\n  ✗ Deploy failed: ${msg}`);
          // WASM 및 ledger 오류 디버깅을 돕기 위해 전체 원인 체인을 로깅
...
메인 상호작용 루프 아래에 Docker 포트 매핑을 생성합니다:
...
`counter-cli/src/standalone.ts`를 생성합니다:
