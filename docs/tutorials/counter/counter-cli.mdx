...
이 튜토리얼에서는 [counter 컨트랙트 구축](./smart-contract) 튜토리얼에서 생성한 counter 스마트 컨트랙트와 상호작용하는 명령줄 인터페이스를 구축하는 방법을 설명합니다.
...
이 섹션에서는 루트 패키지를 설정하는 과정을 설명합니다.
...
DUST 모니터링 함수는 수수료 토큰 가용성에 대한 실시간 피드백을 제공합니다.
사용 가능한 잔액, 코인 수, 대기 중인 트랜잭션 및 등록 상태를 표시합니다.
모니터는 5초마다 업데이트되며 사용자가 Enter 키를 누를 때까지 실행되어 DUST 생성 진행 상황에 대한 가시성을 제공합니다.
...
DUST 잔액은 메뉴 헤더에 동적으로 표시되어 수수료 토큰 가용성에 대한 지속적인 피드백을 제공합니다.
...
배포 흐름은 메시지와 원인 체인을 표시하여 오류를 정상적으로 처리합니다.
DUST 관련 오류는 사용자에게 모니터링 옵션으로 안내하는 유용한 메시지와 함께 특별한 처리를 받습니다.
