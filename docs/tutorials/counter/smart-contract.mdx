...
`CompactStandardLibrary`를 통해 카운터 값을 증가 및 감소시키는 `Counter` 타입 등 내장 타입과 함수를 사용할 수 있습니다.
...
- `export`를 사용하면 TypeScript API 및 컨트랙트의 JavaScript 구현에서 ledger 상태에 접근할 수 있습니다.
...
- `export circuit`는 TypeScript API 및 JavaScript 구현에서 호출할 수 있는 진입점으로 표시합니다.
...
- `circuits` 필드로 외부 상태를 수정하지 않는 순수 circuit 함수에 접근합니다.
- `impureCircuits` 필드로 witness와 상호작용할 수 있는 불순 circuit 함수에 접근합니다.
...
Witness는 circuit 실행 중 private 데이터에 접근하는 역할이지만, 이 간단한 컨트랙트에서는 필요하지 않습니다.
...
- 사용하는 곳에서 하나의 임포트 포인트로 접근할 수 있습니다
