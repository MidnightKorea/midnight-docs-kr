...
Vercel 빌드 설정을 만듭니다. 이 설정은 Vercel에게 모노레포를 빌드하는 방법을 알려줍니다. 먼저 contract와 API 패키지를 컴파일한 다음, Vite 프론트엔드를 빌드합니다.
...
이 아키텍처는 어떤 Midnight DApp에도 확장해 적용할 수 있습니다. 컨트랙트는 온체인 로직을 정의합니다. API 레이어는 이를 TypeScript에서 쓸 수 있게 감쌉니다. 프론트엔드는 Lace를 통해 연결되고, 지갑의 proof 서버가 proof 생성을 처리합니다.
