...
witness 없이 비공개 데이터를 다루는 안전한 Compact contract를 작성하고, MidnightJS 기반 로컬 devnet 테스트 스크립트로 contract의 동작을 검증하는 과정을 다룹니다. 코드를 복사해 붙여 넣기보다는 각 코드 블록을 직접 타이핑하며 따라가세요.
...
예를 들어 ledger 쓰기, export된 circuit의 반환값, contract 간 호출, Unshielded token 전송은 모두 공개됩니다. 앞의 세 가지 영역을 지나는 데이터는 Compact의 해싱과 commitment 기법으로 가릴 수 있습니다. 그렇다면 Unshielded token은 어떨까요?
...
organizer 역시 contract가 청구된 입장료를 지급하기 전까지는 DApp 안에서 privacy를 유지합니다.

### Operational steps

먼저 contract의 동작 측면을 생각해 봅니다. contract는 무엇을 해야 할까요?

순서대로 정리하면 다음과 같습니다:
1. contract를 배포하고 organizer가 초기값을 설정
...
Compact는 데이터를 기본적으로 비공개로 취급합니다. 이 애플리케이션에서는 비공개 상태 데이터를 설정하고 Compact circuit에 비공개로 전달하는 과정을 보여 줍니다. Unshielded token 전송이 일어나기 전까지는 어떤 참석자 데이터도 공개되지 않습니다.
...
Midnight 블록체인 같은 개방형 공개 시스템에서 smart contract를 개발할 때 중요한 요소 중 하나가 circuit 접근 제어입니다. contract를 블록체인에 배포하는 순간, 그 circuit는 사실상 *누구나* 호출할 수 있는 공개 API가 됩니다. 안전한 Compact contract라면 특정 사용자나 특정 유형의 사용자만 쓰도록 만든 circuit의 접근을 보호해야 합니다.
...
이 contract는 witness 함수를 사용하지 않지만, witness는 circuit 안에서 비공개 상태 데이터를 수정하거나, 오프체인 연산을 제공하거나, 나눗셈처럼 Compact에 없는 기능을 구현할 때 유용합니다. witness는 Compact에서 선언만 하고 구현은 오프체인 TypeScript 코드에서 하므로, witness의 반환값은 사전 검증 없이는 신뢰할 수 없습니다. witness 함수의 예시는 [Battleship 튜토리얼](../bship/smart-contract#witnesses)을 참고하세요.

이 튜토리얼에서는 비공개 상태 데이터에 Compact circuit 입력으로 접근합니다.

### Constructor

다음으로 contract 배포 시점에 실행되는 constructor를 작성합니다. 이 contract는 organizer가 배포합니다.
...
`getDappPublicKey()`의 구현은 뒤에서 다룹니다. 지금은 organizer가 제공한 secret을 해싱해 이 DApp 전용 "public key"를 만든다는 것만 이해하면 됩니다. Compact circuit 입력은 비공개이므로, `_secret`은 비공개 상태에서 circuit 입력으로 곧바로 전달되며 외부에 노출되지 않습니다. `pubKey`는 organizer의 주소나 다른 식별 정보와 연결되지 않으므로 ledger에 공개로 저장해도 안전합니다.
...
organizer가 자기 파티에 참석하려는 것이 아닌지 `assert`로 확인한 다음, contract가 RSVP를 받을 수 있는 올바른 상태인지 검증합니다.
...
이 circuit는 호출자의 `pubKey`를 식별하고, 앞서 저장한 organizer의 public key와 일치하는지 확인한 뒤, contract 상태가 두 가지 허용 값 중 하나인지 검사하고 나서 상태를 갱신합니다. contract 상태가 바뀌면 참석자가 파티에 check-in할 수 있습니다.
...
호출자를 식별했고 상태도 기대한 대로이니, 이제 contract가 결제를 받을 차례입니다. 파티 입장료는 NIGHT으로 결제하며, NIGHT은 Unshielded token입니다. Unshielded token 거래는 모두 공개됩니다.
...
contract는 받은 NIGHT token을 organizer가 청구할 때까지 보관합니다. 이제 organizer가 이 입장료를 청구할 수 있는 circuit가 필요합니다:
```compact
...
이 circuit는 앞의 circuit들과 구성이 비슷하지만, token을 organizer에게 보내기 전에 사용 가능한 잔액을 계산한다는 점이 다릅니다. `sendUnshielded()` 함수를 거치면서 organizer도 공개됩니다.

`unshieldedBalanceGte()`로 contract 잔액이 기대한 값인지 확인하는 일은 중요합니다. 잔액이 `sendUnshielded()` 트랜잭션을 수행하기에 부족하면 circuit 실행이 실패합니다.
...
컴파일을 통과했다는 것은 Compact 컴파일러가 보기에 문법이 올바르다는 뜻일 뿐입니다. contract가 기대대로 동작하는지 확인하려면 테스트 스위트를 실행하세요. 안전한 contract를 작성하려면 포괄적인 테스트 스위트가 필수입니다.
...
proof server는 Midnight의 zero-knowledge proof 시스템에서 핵심적인 구성 요소입니다. 자세한 내용은 [Zero-knowledge proofs](../../concepts/zero-knowledge-proofs.mdx)를 참고하세요.
...
이것으로 private party 튜토리얼을 마칩니다. Unshielded NIGHT token을 사용하면서 참석자가 공개되었습니다. 참석자의 privacy를 끝까지 유지하고 싶다면, 이 contract와 테스트 스위트를 Shielded token을 사용하도록 고쳐 보세요.
