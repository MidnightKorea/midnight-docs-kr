...
smart contract는 컴파일됐고, attestation API는 신용 데이터에 서명할 수 있으며, proof server는 zero-knowledge(ZK) proof를 생성할 수 있습니다. 이제 이 셋과 모두 대화할 방법이 필요합니다.
...
1.  **CLI**: 지갑 생성, smart contract 배포, attestation provider 등록, 대출 요청, on-chain 상태 조회를 처리하는 대화형 명령줄 도구.

2.  **End-to-end 테스트**: 전체 시스템을 직접 돌려 보는 과정입니다. 지갑에 tNIGHT를 충전하고, smart contract를 배포하고, provider를 등록하고, 비공개 신용 데이터로 대출을 요청한 뒤, on-chain에는 결과만 드러나는지 확인합니다.

### Prerequisites

- [Part 1](./smart-contract)과 [Part 2](./attestation-api)를 완료해야 합니다
- 컴파일된 smart contract 패키지가 `contract/dist/`에 있어야 합니다
- attestation API를 시작할 준비가 되어 있어야 합니다
- Docker proof server가 `6300` 포트에서 실행 중이어야 합니다
- Node.js v22 이상 — Node 20에서는 지갑이 시작되는 듯 보이다가 동기화 중 `TypeError: state.pendingOutputs.values.map is not a function` 오류로 죽습니다([Part 1](./smart-contract)의 주의 사항을 참고하세요)

## Build the CLI

CLI는 지갑 작업, smart contract 상호작용, attestation 요청을 조율하는 TypeScript 애플리케이션입니다. 각각 역할이 분명한 7개 파일로 구성됩니다.

### Configuration

CLI는 대상 네트워크가 Midnight의 원격 Preprod 인프라든 로컬 개발 네트워크든, 해당 네트워크의 indexer, 블록체인 node, proof server 위치를 알아야 합니다. 또한 [Part 1](./smart-contract)에서 컴파일한 circuit 산출물 경로도 필요합니다.
...
이 파일은 두 네트워크를 모두 다룹니다. `PreprodConfig`는 indexer와 node를 Midnight의 원격 인프라로 향하게 하고, `LocalDevConfig`는 로컬 스택으로 향하게 합니다. 두 경우 모두 proof server는 로컬 `6300` 포트에서 실행됩니다. Preprod라면 [Part 2](./attestation-api)에서 띄운 Docker 컨테이너이고, 아니면 [Midnight Local Dev](/guides/midnight-local-network)에 포함된 컨테이너입니다.
...
Midnight JS SDK는 타입이 매우 촘촘합니다. smart contract와 상호작용할 때마다 circuit, private state, provider 번들에 대한 구체적인 타입 매개변수가 필요합니다. 이 타입들을 파일마다 반복하는 대신, 이 모듈에서 한 번만 정의하고 export해 CLI 전반에서 사용합니다.
...
*   `ZKLoanCreditScorerProviders`는 Midnight JS SDK가 요구하는 6개 provider 타입을 묶습니다. wallet, midnight(트랜잭션 제출), proof, ZK config, public data(indexer), private state(LevelDB)입니다.

*   `DeployedZKLoanCreditScorerContract`가 union인 이유는, 새 smart contract를 배포하거나(`DeployedContract` 반환) 기존 smart contract에 합류할 수(`FoundContract` 반환) 있기 때문입니다. 둘 다 동일한 `callTx` 인터페이스를 노출합니다.
...
이 프로필들은 [Part 1](./smart-contract)의 smart contract에 정의된 자격 등급과 대응됩니다:
...
이 핵심 모듈은 CLI를 Midnight SDK에 연결합니다. 네 가지 일을 맡습니다. BIP-39 mnemonic으로 지갑을 만들고 충전하기, 설정된 네트워크에서 smart contract를 배포하거나 합류하기, [Part 2](./attestation-api)에서 만든 API로부터 Schnorr attestation 받아 오기, 그리고 각 smart contract circuit 호출(대출 요청, PIN 변경, admin 작업)을 대화형 CLI가 부를 수 있는 함수로 감싸기입니다. 분량이 크므로 논리 단위로 나누고 블록마다 설명을 덧붙입니다.
...
`getZKLoanLedgerState`는 indexer에 smart contract의 현재 on-chain 상태를 질의합니다. `ZKLoanCreditScorer.ledger()` 함수는 원시 contract 상태를 `contractAdmin`, `loans`, `providers`, `blacklist` 같은 필드를 가진 타입이 지정된 `Ledger` 객체로 역직렬화합니다.

#### Compiled smart contract and deploy/join

이 섹션은 컴파일된 contract 산출물을 witness 구현과 결합한 뒤, 새 contract를 배포하거나 기존 contract에 합류하는 함수를 제공합니다:
...
*   `deploy`는 설정된 네트워크에 smart contract의 새 인스턴스를 만듭니다. ZK proof가 포함된 배포 트랜잭션을 일으키는데, 이 proof는 proof server가 생성하며 약 1분이 걸립니다.

*   `joinContract`은 주소로 이미 배포된 smart contract에 연결합니다. 두 번째 사용자(또는 새 세션의 같은 사용자)가 다른 사람이 배포한 smart contract와 상호작용하는 방법입니다.
...
4.  smart contract의 `requestLoan`을 호출합니다. proof server가 private state(attestation과 user secret 포함)를 읽어 ZK proof를 생성합니다.

`computeUserPubKeyHash` 함수는 smart contract가 on-chain에서 쓰는 것과 동일한 `deriveUserPublicKey` pure circuit을 사용합니다. 생성된 `pureCircuits.deriveUserPublicKey`를 TypeScript에서 호출하면 off-chain 해시와 in-circuit 해시가 일치하도록 보장되므로, attestation 메시지와 proof 안의 신원이 맞아떨어집니다.
...
proof 생성, 트랜잭션 balancing, 제출은 SDK가 뒤에서 처리합니다.

#### Wallet and provider infrastructure

아래 코드 블록은 `createWalletAndMidnightProvider`를 정의합니다. 지갑이 indexer와 동기화될 때까지 기다린 뒤, SDK가 트랜잭션을 balance하고 제출하는 데 쓰는 wallet 및 Midnight provider 결합본을 반환합니다. 더 넓은 범위의 지갑 설정(자금 폴링, tDUST 생성을 위한 tNIGHT UTXO 등록)은 이어지는 섹션에서 다룹니다.
...
`configureProviders`는 6개 SDK provider(wallet, Midnight, proof, ZK config, public data, private state)를 하나의 번들로 조립합니다. `setLogger`와 `closeWallet` 유틸리티는 모듈 레벨 logger를 관리하고 종료 시 지갑 리소스를 정리합니다.
...
API 모듈이 모든 SDK 상호작용을 처리하므로, CLI 모듈은 사용자 대면 레이어를 맡습니다. 입력을 받고, 선택을 올바른 API 함수로 연결하고, 오류가 나도 멈추지 않게 처리합니다. 대화형 터미널 입력에는 Node 내장 `readline/promises`를 사용합니다.
...
- `DEPLOY_OR_JOIN_QUESTION`: 지갑 설정 후 표시되는 첫 메뉴로, 새 contract를 배포할지 기존 contract에 합류할지 묻습니다.
- `MAIN_LOOP_QUESTION`: contract를 배포하거나 합류한 뒤 표시되는 메인 메뉴로, 대출 및 admin 작업을 나열합니다.
...
이 함수들은 사용자에게 새 contract를 배포할지, 주소로 기존 contract에 연결할지 묻습니다:
...
blacklist 흐름은 지갑 주소가 아니라 파생된 `UserPublicKey`(64 hex 문자)를 받습니다. admin은 이 값을 on-chain `loans` map에서 읽거나(상호작용한 사용자의 키로 나타남) 대상에게서 별도 경로로 얻습니다. admin은 아직 contract를 건드리지 않은 사용자를 선제적으로 blacklist에 올릴 수 없습니다 — 위조 불가능한 호출자 신원을 위한 의도적인 트레이드오프입니다.
...
- **Deploy or join**: 새 smart contract를 배포하거나, 주소로 기존 contract에 연결합니다.
...
마지막 파트에서는 [Midnight Local Dev](/guides/midnight-local-network)를 사용해 전체 흐름을 테스트하며 모든 것을 하나로 묶습니다. Midnight Local Dev는 Midnight node, indexer, proof server를 로컬에서 실행하는 독립 실행형 Docker 환경입니다. `npm start` 마법사는 원하는 주소로 NIGHT를 전송할 수 있고, 그러면 ZKLoan CLI가 그 NIGHT를 DUST용으로 자동 등록하므로 지갑은 트랜잭션을 제출할 준비를 마칩니다.
...
이 명령은 Docker 이미지를 받아 와 node, indexer, proof server를 각각 `9944`, `8088`, `6300` 포트에서 시작합니다. 이어서 genesis master 지갑을 초기화하고 대화형 메뉴를 보여 줍니다. 이미 네트워크가 실행 중이면 마법사가 먼저 그것을 재사용할지, 새 이미지로 다시 시작할지 묻습니다 — 어느 쪽이든 선택하세요.
...
wallet 메뉴가 나타나면 **옵션 1(Build a fresh wallet)**을 고르세요. CLI가 24단어 mnemonic을 생성하고, 지갑을 파생한 뒤, 다음을 출력합니다:
...
`mn_shield-addr_undeployed1q…` 주소를 복사하세요. **첫 번째 터미널**(local-dev 메뉴)로 전환해 **`[2] Fund accounts by public key (NIGHT transfer only)`**를 고른 뒤 주소를 붙여 넣으세요. 마법사가 그 주소로 50,000 NIGHT를 전송합니다.
...
이제 CLI가 deploy/join 메뉴를 보여 줍니다. **옵션 1(Deploy a new ZKLoan Credit Scorer contract)**을 고르세요. proof server가 배포 proof를 생성하는데, 약 1분이 걸립니다.

완료되면 CLI가 `Deployed contract at address: 02…`를 로그로 남기고 메인 10개 옵션 메뉴로 넘어갑니다.

#### 3. Register the attestation provider

이 단계는 contract마다 한 번만 합니다. 이 단계를 거치지 않으면 circuit이 provider 등록 여부를 assert하므로 모든 대출 요청이 revert됩니다.

1. **옵션 8(Register attestation provider)**을 고르세요.
2. provider ID로 `1`을 입력하세요.
3. 두 번째 터미널에서 attestation API가 출력한 `x`와 `y` 좌표를 붙여 넣으세요.

이제 provider의 public key가 contract의 `providers` map에 들어갔습니다.
...
CLI는 API에서 attestation을 받아 로컬 private state에 저장한 뒤, 대출 요청을 제출합니다. 약 1분 뒤 proof가 완료되고 트랜잭션이 finalize됩니다.

#### 5. Inspect the on-chain state

**옵션 3(Display contract state)**을 고르세요. CLI가 contract 주소, admin public key(배포한 admin의 user secret 해시), blacklist 크기, 그리고 대출 기록(상태 + 승인 금액)을 로그로 남깁니다.
