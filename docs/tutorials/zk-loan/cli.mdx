...
[Part 1](./smart-contract)에서는 Compact smart contract와 witness 레이어를 만들었고, [Part 2](./attestation-api)에서는 attestation API를 만들고 proof server를 설정했습니다. 이 마지막 파트에서는 모든 것을 하나로 묶는 CLI를 구축한 뒤, Midnight의 Preprod 네트워크에서 전체 흐름을 end-to-end로 실행합니다.
...
CLI는 지갑 작업, smart contract 상호작용, attestation 요청을 조율하는 TypeScript 애플리케이션입니다. 각각 역할이 분명한 6개 파일로 구성됩니다.

### Configuration

CLI는 Midnight Preprod indexer, 블록체인 node, 로컬 proof server의 위치를 알아야 합니다. 또한 [Part 1](./smart-contract)에서 컴파일한 circuit 산출물 경로도 필요합니다.
...
여기에는 Preprod와 Local Dev 설정이 모두 들어 있습니다. indexer와 node는 Midnight의 원격 인프라를 가리키고, proof server는 [Part 2](./attestation-api)에서 띄운 로컬 Docker 컨테이너를 가리킵니다.
...
이 핵심 모듈은 CLI를 Midnight SDK에 연결합니다. 네 가지 일을 맡습니다. BIP-39 mnemonic으로 지갑을 만들고 충전하기, Preprod에서 smart contract를 배포하거나 합류하기, [Part 2](./attestation-api)에서 만든 API로부터 Schnorr attestation 받아 오기, 그리고 각 smart contract circuit 호출(대출 요청, PIN 변경, admin 작업)을 대화형 CLI가 부를 수 있는 함수로 감싸기입니다. 분량이 크므로 논리 단위로 나누고 블록마다 설명을 덧붙입니다.
...
*   `deploy`는 Preprod에 smart contract의 새 인스턴스를 만듭니다. ZK proof가 포함된 배포 트랜잭션을 일으키는데, 이 proof는 proof server가 생성하며 약 1분이 걸립니다.
...
*   **NightExternal**: faucet에서 tNIGHT 받기 같은 unshielded(transparent) 작업에 사용
...
진입점은 Preprod 설정을 초기화하고, logger를 만든 뒤, 둘 다 CLI 러너에 넘깁니다.
