...
`compact-js@2.5.0`은 `compact-runtime@0.15.0`을 정확히 고정(exact-pin)하지만, 이 고정은 compact-js 내부 의존성 해석에만 적용됩니다. npm은 `0.15` 빌드를 `node_modules/@midnight-ntwrk/compact-js/node_modules/` 안에 중첩해 두고, 최상위에서는 애플리케이션 코드에 `0.16`을 제공합니다. 워크스페이스 수준의 버전 충돌은 없으며, 중첩된 사본에 대해 따로 조치할 필요는 없습니다.
...
`new type`은 그것이 대체하는 단일 필드 wrapper struct(`struct UserPublicKey { bytes: Bytes<32>; }`)보다 가볍습니다. 객체 wrapper를 더하지 않으므로 값은 Compact에서는 그대로 `Bytes<32>`, 생성된 TypeScript에서는 `{ bytes: ... }` 객체가 아닌 일반 `Uint8Array`입니다. `UserPublicKey { ... }`나 `UserPublicKey(...)` 같은 생성자는 없습니다. 타입으로 *들어갈* 때는 `value as UserPublicKey`, 원래 바이트로 *나올* 때는 `pk as Bytes<32>`를 씁니다. exported circuit나 ledger 시그니처에 등장하는 `new type`에는 `export`를 붙여서 컴파일러가 TypeScript 바인딩을 생성하게 하세요.
...
- `blacklist`는 wallet 주소가 아니라 도출된 `UserPublicKey` 값을 저장합니다. 특정 PIN에서 사용자의 `UserPublicKey`는 `persistentHash("zkloan:user:pk:v1" || hash(pin) || userSecret)`입니다. 악의적인 caller는 다른 wallet pubkey를 주장해 blacklist 검사를 우회할 수 없습니다. 이 contract에서 그들의 신원은 자신의 witness secret이 해시된 결과 그 자체이기 때문입니다.
- `onGoingPinMigration`은 사용자가 PIN을 바꿀 때 진행 상황을 추적합니다(자세한 내용은 신원 섹션에서 다룹니다).
- `deriveUserPublicKey(sk, pin)`은 사용자별 신원 도출이고, `deriveAdminPublicKey(sk)`는 admin 도출입니다. 둘 다 동일한 32바이트 witness secret을 사용합니다. 서로 다른 도메인 분리 문자열이 두 도출 결과가 연관되지 않게 합니다.
...
*   `getUserSecret`: 이 contract의 모든 caller 신원을 만들어 내는 32바이트 secret을 반환합니다. `UserSecretKey`가 이제 `Bytes<32>`에 대한 `new type`이므로, 생성된 TypeScript 타입은 일반 `Uint8Array`입니다. witness는 `{ bytes: ... }` wrapper 없이 secret 자체를 반환합니다. contract는 이 값을 두 개의 pure circuit에 넣습니다. 사용자별 PIN 바인딩 신원을 위한 `deriveUserPublicKey(secret, pin)`과 admin 역할을 위한 `deriveAdminPublicKey(secret)`입니다. `contractAdmin`에 저장된 값으로 해시되는 secret은 배포한 admin의 것뿐입니다. 다른 사람의 `getUserSecret`은 admin 도출 결과가 일치하지 않는 값을 반환하므로, 그들에게는 admin assertion이 실패합니다. 도메인 분리 문자열(`"zkloan:admin:pk:v1"`과 `"zkloan:user:pk:v1"`)이 같은 secret에서 나온 두 도출 결과가 연관되지 않게 합니다. 런타임 길이 검사로 잘못된 형식의 witness는 proof 시점에 거부됩니다.
