...
`UserSecretKey`, `UserPublicKey`, `AdminPublicKey`는 각각 하나의 `Bytes<32>`를 감쌉니다. 이 wrapper struct는 의도된 설계입니다. 셋 다 32바이트 값이지만, 각각에 별개의 이름 있는 타입을 부여하면 Compact에서 *명목상* 서로 다른 타입이 됩니다. 그래서 컴파일러는 `AdminPublicKey`가 와야 할 자리에 `UserPublicKey`를 넘기는 것을(그 반대도) 거부합니다. 어디서나 그냥 `Bytes<32>`를 써도 컴파일은 되지만, 그러면 user key와 admin key를 서로 바꿔 쓸 수 있게 됩니다. 이 contract의 보안은 바로 그런 혼동을 막는 데 달려 있습니다.
...
*   `getUserSecret`: 이 contract의 모든 caller 신원을 만들어 내는 32바이트 secret을 반환합니다. contract는 이 값을 두 개의 pure circuit에 넣습니다. 사용자별 PIN 바인딩 신원을 위한 `deriveUserPublicKey(secret, pin)`과 admin 역할을 위한 `deriveAdminPublicKey(secret)`입니다. `contractAdmin`에 저장된 값으로 해시되는 secret은 배포한 admin의 것뿐입니다. 다른 사람의 `getUserSecret`은 admin 도출 결과가 일치하지 않는 값을 반환하므로, 그들에게는 admin assertion이 실패합니다. 도메인 분리 문자열(`"zkloan:admin:pk:v1"`과 `"zkloan:user:pk:v1"`)이 같은 secret에서 나온 두 도출 결과가 연관되지 않게 합니다. 런타임 길이 검사로 잘못된 형식의 witness는 proof 시점에 거부됩니다.
