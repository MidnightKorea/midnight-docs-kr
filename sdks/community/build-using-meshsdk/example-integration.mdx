...
사전 구축된 스마트 컨트랙트, 완전한 API, 바로 사용할 수 있는 코드 스니펫으로 Midnight Network 위에서 가장 빠르게 개발을 시작하는 방법입니다.

## Try the project

전체 설정을 로컬에서 테스트하려면 다음 단계를 정확히 따라 진행하세요:

**Repository:** [https://github.com/MeshJS/midnight-setup](https://github.com/MeshJS/midnight-setup)

### Clone the repository

HTTPS 또는 SSH를 사용하여 저장소를 로컬에 복제하세요.
...
Docker Compose로 로컬 테스트넷 서비스를 시작하세요.
...
컨트랙트 API와 상호작용하기 전에 필요한 프로바이더를 설정하세요.
...
SDK에서 사용할 수 있는 핵심 컨트랙트 워크플로를 보여주는 예제입니다.

### Deploy a contract

새 컨트랙트 인스턴스를 배포하고 주소를 확인합니다.
...
아래 훅과 컴포넌트 패턴을 사용하여 SDK를 React 앱에 연결하세요.

### Custom hook

배포 및 참여 흐름을 재사용 가능한 React 훅으로 래핑합니다.
...
아래 패턴을 사용하여 일반적인 SDK 오류를 표시하고 UI를 보호하세요.

### Common error patterns

알려진 SDK 오류를 사용자 친화적인 메시지로 매핑합니다.
