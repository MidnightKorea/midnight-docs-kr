...
| Class | Kind | Trigger |
| --- | --- | --- |
| `TxFailedError` | 트랜잭션 | 합의 노드가 트랜잭션을 거부하거나 적용에 실패한 경우 |
...
| `ScopedTransactionIdentityMismatchError` | State / identity | 스코프드 배치가 다른 컨트랙트나 private state의 캐시를 재사용하려는 경우 |
...
- `circuitId`(선택): 트랜잭션을 구성할 때 사용된 circuit
...
- `circuitId`: 트랜잭션을 구성할 때 사용된 circuit
...
- `circuitIds`: 정의되지 않았거나 검증자 키가 일치하지 않는 circuit
...
- `privateStateId`: 호출 트랜잭션에 지정된 private state ID
- `privateStateProvider`: 호출 트랜잭션에 지정된 private state provider
...
- `initialPrivateState`: 컨트랙트 조회에 지정된 초기 private state
- `privateStateId`: 컨트랙트 조회에 지정된 private state ID
...
스코프드 트랜잭션이 다른 컨트랙트 주소나 private state ID로 캐시된 상태를 재사용하려고 할 때 발생합니다.

#### Properties

- `cached`: 캐시에 저장된 컨트랙트 주소와 private state ID
- `requested`: 요청된 컨트랙트 주소와 private state ID
