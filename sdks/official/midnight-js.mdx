...
Midnight.js는 스마트 컨트랙트를 배포하고 호출하며, 암호화된 private state를 관리하고, 영지식 증명을 생성해 Midnight 네트워크에 트랜잭션을 제출하는 도구를 제공합니다.
...
provider 패키지는 증명 생성, private state 관리, 공개 데이터 조회 기능을 담당합니다.
...
`deployContract` 메서드는 스마트 컨트랙트를 블록체인에 배포합니다. 매개변수는 다음과 같습니다.

- `providers`: provider 객체
- `compiledContract`: 컴파일된 컨트랙트
- `privateStateId`: private state ID
- `initialPrivateState`: 초기 private state
...
`deployContract` 메서드는 `deployedContract` promise를 반환합니다. 이 객체에는 컨트랙트 주소와 private state ID가 담겨 있습니다.
...
컨트랙트 객체를 초기화하고 나면 `callTx` 속성으로 스마트 컨트랙트의 circuit을 호출할 수 있습니다.
...
스마트 컨트랙트를 배포한 뒤에는 `getStates`와 `getPublicStates` 메서드로 컨트랙트의 private state와 public state를 조회할 수 있습니다.
...
`getStates` 메서드는 지정한 provider를 통해, 주어진 식별자에 해당하는 배포된 스마트 컨트랙트의 Zswap 상태, ledger 상태, private state를 가져옵니다.
...
`getPublicStates` 메서드는 배포된 스마트 컨트랙트에서 외부에 공개되는 상태(Zswap과 ledger)만 가져옵니다. 매개변수는 다음과 같습니다.
