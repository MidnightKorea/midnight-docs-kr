...
description: Midnight Wallet SDK 개발자 가이드로, unshielded, shielded, DUST 지갑 운영을 다룹니다.
...
- **Unshielded tokens**: NIGHT 및 기타 unshielded 토큰
- **Shielded tokens**: 영지식 증명이 적용된 shielded 토큰
...
| `@midnight-ntwrk/wallet-sdk-shielded` | ZK 증명이 적용된 shielded 토큰 관리 |
...
| `@midnight-ntwrk/wallet-sdk-prover-client` | proof server와의 인터페이스 |
...
- **UnshieldedWallet**: UTxO 모델 기반으로 NIGHT 등 unshielded 토큰을 관리
- **ShieldedWallet**: ZK 증명으로 프라이버시를 보장하는 shielded 토큰을 관리
...
- `role`: 키의 용도를 결정하는 지갑 유형 식별자:
  - `0` (Roles.NightExternal): unshielded 운영
  - `3` (Roles.Zswap): shielded 운영
  - `4` (Roles.Dust): DUST 토큰 운영
- `index`: 주소 인덱스로, 일반적으로 기본 주소는 0

### Derive keys from a seed

하나의 마스터 시드에서 unshielded, shielded, DUST 키를 파생하는 예제입니다. 키 파생이 실패하면 BIP-44 표준에 따라 다음 인덱스로 자동 재시도합니다.
...
|--------|------|-------------|
| `mn_addr` | Unshielded | NIGHT 및 unshielded 토큰의 결제 주소 |
| `mn_shield-addr` | Shielded | shielded 토큰의 결제 주소 |
...
표준 UTxO 기반 트랜잭션으로 주소 간에 NIGHT 등 unshielded 토큰을 전송합니다. 블록체인에 공개되며, unshielded 비밀 키로 서명해야 합니다.
...
unshielded 전송은 Schnorr 서명을 사용하므로 unshielded 비밀 키가 필요합니다.

### Shielded transfers

영지식 증명으로 토큰을 비공개 전송합니다. 트랜잭션 금액과 참여자 정보가 외부에 노출되지 않으면서도 정확성은 검증 가능합니다. unshielded 서명은 필요하지 않습니다.
...
:::note
shielded 전송은 모든 작업이 영지식 증명으로 검증되므로 별도의 unshielded 서명이 필요 없습니다.
:::

### Combined transfers

unshielded 전송과 shielded 전송을 하나의 원자적 트랜잭션으로 결합할 수 있습니다. 서로 다른 토큰 유형을 한 번에 전송하므로 오버헤드가 줄고, 둘 다 성공하거나 둘 다 실패합니다.
...
DUST은 등록된 NIGHT에서 생성되며 트랜잭션 수수료 지불에 사용됩니다. 아래 예제는 unshielded 키스토어와 비밀 키가 준비된 상태를 전제합니다.

### Register NIGHT for DUST generation

unshielded NIGHT 코인을 등록하면 시간이 지나면서 DUST 토큰이 생성됩니다. 생성된 DUST은 DUST 지갑에 자동 축적됩니다. 등록한 NIGHT은 unshielded 지갑에 그대로 남아 있으며, 언제든 등록을 해제할 수 있습니다.
...
기본적으로 HTTP 기반 proof server를 사용하지만, 브라우저 환경 등에서는 WASM 기반 증명으로 대체할 수 있습니다.

### WASM proving

브라우저 환경이나 proof server에 HTTP 접근이 불가능한 경우 WebAssembly 기반 증명을 사용하세요. 클라이언트에서 증명을 직접 생성하지만 전용 proof server보다 훨씬 느립니다.
...
:::warning Performance
WASM 증명은 네이티브 proof server보다 느립니다. proof server에 HTTP 접근이 불가능한 경우에만 사용하세요.
:::
...
지갑을 시작하면 노드 및 indexer 연결, 트랜잭션 및 상태 스캔 등 Midnight Network 동기화가 시작됩니다. wallet facade 초기화 직후, 다른 작업 전에 호출하세요.
